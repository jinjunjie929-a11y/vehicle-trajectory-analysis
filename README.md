...
一个基于道路监控视频的车辆检测与轨迹分析项目。项目使用 YOLO 完成车辆检测，结合 ROI 区域、稳定车道线建模和车辆 ID 跟踪，输出车辆逐帧轨迹、右侧车道线距离、Excel 表格和轨迹可视化图片。

本仓库面向 GitHub 展示和面试交流，重点体现 Python 视觉工程中的配置管理、处理流程编排、结构化结果导出和可视化展示能力。
...
道路监控视频中，车辆检测和轨迹分析会受到摄像机视角、车道线遮挡、远处小目标和 ROI 范围变化的影响。项目将完整流程拆成多个可复用步骤：

1. 读取视频信息并截取参考帧。
2. 通过 ROI 配置限定道路区域。
3. 在 ROI 内生成稳定车道线地图。
4. 使用 YOLO 检测车辆并记录逐帧位置。
5. 进行车辆 ID 跟踪和右侧车道线距离计算。
6. 将车辆轨迹转换到 ROI 相对坐标系。
7. 导出 Excel、CSV 和轨迹图。
...
为避免上传大文件，本仓库不包含以下本地数据目录和运行结果：

- `data/`：原始视频文件。
- `models/`：YOLO 权重文件。
- `outputs/`：运行生成的 CSV、Excel、图片和视频结果。
- `debug/`：调试帧、ROI mask、车道线预览等中间文件。

使用前需要自行准备输入视频和 YOLO 权重，并按 `config.yaml` 中的路径放置。
...
- YOLO 权重文件和原始视频较大，建议只在本地保留，不提交到 GitHub。
